...
export function formatUserAdminErrorMessage(error: unknown): string {
  if (error instanceof ApiRequestError) {
    ...
    if (error.code === "NOT_FOUND") {
      return error.message || "User was not found";
    }

    if (error.code === "CONFLICT") {
      return error.message || "Cannot demote the last remaining admin.";
    }
